...
| 2026-09-14 | 이메일 발송을 클라우드 루틴이 아닌 GitHub Actions(`.github/workflows/send-digest-email.yml`)로 이전. Gmail 앱 비밀번호는 GitHub Secrets(GMAIL_USER/GMAIL_APP_PASSWORD/DIGEST_RECIPIENT)에 저장, `reports/security-digest/**.md` push를 트리거로 `scripts/send_digest_email.py` 실행 | .github/workflows, skills/security-news-digest | Claude Code 클라우드 루틴에 시크릿(환경변수) 저장 기능이 없어, 공개 저장소에 비밀번호 노출 없이 자동 발송하려면 GitHub Secrets가 필요했음 |
